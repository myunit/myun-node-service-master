...
module.exports = function (Book) {
  Book.getApp(function (err, app) {
    ...
    Book.getLogisticsInfo = function (company, postId, cb) {
      ...
      goodsInter.getLogisticsInfo(track_company === undefined ? company:track_company.short, postId, function (err, data) {
        if (err != 200) {
    ...
    //完结订单
    Book.finishOrderDelivery = function (data, cb) {
      orderIFS.finishOrderDelivery(data, function (err, res) {
        if (err) {
          console.log('finishOrderDelivery err: ' + err);
          cb(null, {status: 0, msg: '操作异常'});
          return;
        }
    ...
    Book.remoteMethod(
      ...
      {
        description: [
          '完结订单(access token).返回结果-status:操作结果 0 失败 1 成功, msg:附带信息'
        ],
        accepts: [
          {
            arg: 'data', type: 'object', required: true, http: {source: 'body'},
            description: [
              '结束包团(JSON string) {"orderId":int}'
            ]
          }

        ],
        returns: {arg: 'repData', type: 'string'},
        http: {path: '/finish-order-delivery', verb: 'post'}
      }
    );
